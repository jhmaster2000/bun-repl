...
const jszig = await (await fetch('https://raw.githubusercontent.com/oven-sh/bun/main/src/bun.js/javascript.zig')).text();
const nodefbzig = await (await fetch('https://raw.githubusercontent.com/oven-sh/bun/main/src/node_fallbacks.zig')).text();
const jszigchunk = jszig.split('pub const HardcodedModule = enum {').at(-1)!.trim();
const nodefbzigchunk = nodefbzig.split('pub const Map = ComptimeStringMap(FallbackModule, .{').at(-1)!.trim();

let tmp = jszigchunk.split('pub const Map = bun.ComptimeStringMap(');
const enumraw = tmp[0].trim();
tmp = tmp[1].split('pub const LinkerMap = bun.ComptimeStringMap(');
const mapraw = tmp[0].trim();
const linkermapraw = tmp[1].trim().split('pub const DisabledModule = bun.ComptimeStringMap(')[0].trim();

const enumlist = enumraw.split('\n').map(ln => (ln = ln.trim(), ln.slice(2, ln.at(-1) === ',' ? -2 : -1)));
...
const jsziglist = [...enumlist, ...maplist, ...linkermaplist];
...
const builtins = new Set([...new Set([...jsziglist, ...nodefblist])].sort());
builtins.delete('path/win32'); builtins.delete('path/posix');
...
for (const mod of builtins) {
    try {
        ...
        if (props.length === 0 || (props.length === 1 && props[0] === 'default'))
...
console.log('node: modules ->', `'${nodePrefixBuiltins.map(s => s.slice(5)).join("', '")}'\n`);
