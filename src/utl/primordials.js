...
const copyProps = (src, dest) => {
    Array.prototype.forEach.call(Reflect.ownKeys(src), key => {
        if (!Reflect.getOwnPropertyDescriptor(dest, key)) {
            Reflect.defineProperty(dest, key, /** @type {*} */(Reflect.getOwnPropertyDescriptor(src, key)));
        }
    });
};
    ...
    if (Symbol.iterator in unsafe.prototype) {
        ...
        Array.prototype.forEach.call(Reflect.ownKeys(unsafe.prototype), key => {
            if (!Reflect.getOwnPropertyDescriptor(safe.prototype, key)) {
                const desc = /** @type {*} */(Reflect.getOwnPropertyDescriptor(unsafe.prototype, key));
                if (
...
const StringIteratorPrototype = /** @type {*} */(Reflect.getPrototypeOf(StringIterator("")));

const StringPrototypeReplace = Function.prototype.call.bind(String.prototype.replace);
